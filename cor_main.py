...
#
def comp_quant( k_s,K,mutnu,thw,dstar ):
	...
	for i in range(len(K)):
		...
		if kplus < 5.0 or K[i] > 6.0e-4: 
			...
			#fthws = 1.0 / (1.0 + np.exp(100.0*(thw[i]-0.0035))) + 0.5	       		   
                 ...
                 quant.append( (float( fthws*((kplus)**6)*dstar[i]**1.0) ) )                   
			#quant.append( (float( fthws*(kplus/dstar[i])**2)) )  
	return quant
...
if __name__ == '__main__':
        ...
        for rht in rhts:
            ...
            for key in curkeys:
                ...
                if not err:
                      ...
                      mom  = np.array( getattr( thisclass, 'thtu'  ) )    
                      thw  = np.array( getattr( thisclass, 'Twtsu' ) )    
            		# compute the correlation function 
                      quant = comp_quant( k_s,K,mut,thw,mom ) 
                      quant2 = [] 
                      # Add some Reynolds number scaling to the computed function 
                      for q in quant:
                          quant2.append( q*(float(rnum)**(1.5)*1.0e5) )
                      #
    ...
    #m,c  = np.polyfit( dist,crit,1 )
    m,c  = np.polyfit( downdist,downslope,1 )
    print m,c 
    #m,c = -0.021496278072, 1.66213944616
    line = []
    for d in dist:  
    	line.append( d*m + c )
